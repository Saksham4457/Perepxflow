...
const app = {
    ...
    init() {
        ...
        this.initTheme();
        if (!this.user) {
            window.location.href = '../create_profile/profile.html';
        } else {
            this.aiPartner = this.user.aiPartner || null;
            this.showSidebar();
            if (!this.aiPartner) {
                this.navigate('ai-selection');
            } else {
                    ...
                    <div class="flex gap-4">
                        <div class="custom-dropdown">
                            ...
                            <div class="dropdown-menu" id="flow-dropdown-menu">
                                ...
                                <div class="dropdown-item border-t border-white/10" onclick="app.setCustomDuration()">Custom Duration...</div>
                            </div>
                        </div>
    ...
    setCustomDuration() {
        this.toggleDropdown();
        document.getElementById('custom-duration-modal').classList.remove('hidden');
        document.getElementById('custom-duration-input').focus();
    },

    closeCustomModal() {
        document.getElementById('custom-duration-modal').classList.add('hidden');
    },
    ...
    applyCustomDuration() {
        ...
        if (mins && !isNaN(mins) && parseInt(mins) > 0) {
            ...
            this.selectDuration(numMins, `${numMins} Min (Custom)`);
                    ...
                    <div style="display: flex; gap: 1rem; margin-top: 2rem; height: 200px; align-items: flex-end;">
                        ${stats.hours.map((h, i) => `
                            <div style="flex: 1; display: flex; flex-direction: column; align-items: center; gap: 8px; position: relative;">
                                ...
                                <div style="width: 15px; background: var(--secondary); border-radius: 4px; height: ${(stats.partner_hours ? stats.partner_hours[i] : 0) * 20}px; position: absolute; bottom: 30px; opacity: 0.3;"></div>
